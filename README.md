...
```
db.getCollection('restaurants').find({"grades.score": {$gte: 70}});
```
- Find all restaurants in `Brooklyn`that have a score greater than `80`
```
db.getCollection('restaurants').find({$and: [{"borough": "Brooklyn"}, {"grades.score": {$gt: 80}}]});
```
*shows no results. Change to 70 to see the only one in the DB*

- All restaurants with `Chilean` or `Czech` cuisine.
```
db.getCollection('restaurants').find({$or: [{"cuisine": "Chilean"}, {"cuisine": "Czech"}]});
```
- All restaurants with grade `A` in **second** position of the array.
```
db.getCollection('restaurants').find({"grades.1.grade": "B"});

db.restaurants.aggregate([ { $project : { grades : 1} } ]);
```
- All restaurants with grades `A` or `B`.
```
db.restaurants.find({$and: [ {"grades.grade": "Z"}, {"grades.grade": "B"} ]});
```
- All restaurants that have a review made in `2014-09-16`.
```
db.restaurants.find({"grades.dates" : ISODate(`2014-09-16`)});
```
...
```
- How many restaurants have been graded after .
```
db.restaurants.find({"grades.dates" : $gt{ISODate(`2015-01-01`)}});
```
